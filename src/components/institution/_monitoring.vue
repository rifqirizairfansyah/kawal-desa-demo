<template>
  <div>
    <combobox-selected :reportRole="routeName" type="report" />
    <b-row>
      <b-col md="4">
    ...
    <b-table striped hover responsive :fields="fields" :items="data" v-if="data.length != 0" class="mt-1">
      ...
      <template v-slot:cell(IMAGE)="data">
        <img :src="'http://absensi-selfie.pptik.id/' + data.item.IMAGE" width="400" class="img-fluid pointer" v-on:click="showModal('http://absensi-selfie.pptik.id/' + data.item.IMAGE)">
      </template>
    </b-table>
    <b-pagination
      ...
      align="right"
      show/>
    <b-alert variant="danger" show v-if="data.length === 0 && loading === false" class="m10 mt-3">Data tidak tersedia</b-alert>
  </div>
</template>
...
export default {
  ...
  computed: {
    routeName () {
      ...
      const routeName = currentRouteName === 'Institution' ? 'aparat' : ((currentRouteName === 'Village') ? 'warga' : undefined)
      return routeName
    },
    role () {
